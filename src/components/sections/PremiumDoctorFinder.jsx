    ...
    <div className="group w-[280px] bg-white rounded-3xl overflow-hidden border border-[#E2E8F0] hover:border-[#8B1A4A]/40 hover:shadow-[0_20px_60px_rgba(139,26,74,0.12)] transition-all duration-500">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-[#EDF2F7]">
          ...
          <button
            onClick={() => onBook ? onBook(doctor) : openWhatsAppBooking({ doctor })}
            className="flex items-center gap-1.5 text-[#8B1A4A] text-[11px] font-bold uppercase tracking-wider hover:gap-2.5 transition-all cursor-pointer"
          >
      ...
      <div className="relative z-10">
        <ImageAutoSlider
          ...
          renderItem={(doctor) => (
            <DoctorCard
              doctor={doctor}
              onBook={(doc) => openWhatsAppBooking({ doctor: doc })}
            />
          )}
        />
      </div>
