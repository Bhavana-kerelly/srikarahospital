...
export function AppointmentWidget({ currentBranch }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    const branchObj = branches.find(b => b.slug === formData.branch)
    const branchName = branchObj ? branchObj.title : (currentBranch?.title || formData.branch)
    openWhatsAppBooking({
      branch: branchName,
      specialty: formData.specialty,
      patientPhone: formData.phone,
      message: `Hello Srikara Hospitals, I would like to book an appointment${formData.specialty ? ` for ${formData.specialty}` : ''}${branchName ? ` at the ${branchName} branch` : ''}.${formData.name ? ` Patient Name: ${formData.name}.` : ''}${formData.phone ? ` Contact: ${formData.phone}.` : ''}`
    })
    setOpen(false)
  ...
  return (
    <>
      ...
      <a
        href="https://wa.me/919247958308"
        target="_blank"
        rel="noopener noreferrer"
        title="Chat on WhatsApp +91 92479 58308"
